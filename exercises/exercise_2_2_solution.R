...
show_patches(landscape = augusta_nlcd, labels = FALSE, class = c(41, 42, 43))
...
show_cores(landscape = augusta_nlcd, labels = FALSE, class = 42, 
           edge_depth = 1)
...
show_lsm(landscape = augusta_nlcd, class = 42,
         what = "lsm_p_core", labels = FALSE)
...
sampled_pland <- sample_lsm(landscape = augusta_nlcd, y = sample_points, 
                            size = 2000, shape = "circle", what = "lsm_c_cpland") %>% 
...
focal_window <- matrix(1, nrow = 9, ncol = 9)
...
evergreen_edge <- landscapemetrics::get_boundaries(forest_patches[[2]])

plot(evergreen_edge[[1]])
